...
import { Room, RoomEvent, Track, RemoteTrack, RemoteParticipant, LocalParticipant, createLocalVideoTrack, createLocalAudioTrack } from 'livekit-client';
...
export function useWebRTCLiveKit({
  ...
  const startStreaming = useCallback(async (stream: MediaStream) => {
    ...
    try {
      ...
      if (videoTrack) {
        const localVideoTrack = createLocalVideoTrack(videoTrack);
      ...
      if (audioTrack) {
        const localAudioTrack = createLocalAudioTrack(audioTrack);
